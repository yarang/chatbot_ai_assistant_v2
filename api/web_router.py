...
async def login(request: Request):
    ...
    return templates.TemplateResponse(
        request, "login.html", {"bot_username": bot_username}
    )
...
async def delete_chat_room_web(request: Request, room_id: str):
    ...
    if user_id not in settings.admin_ids:
        logger.warning(
            f"Unauthorized delete attempt: user_id={user_id}, room_id={room_id}"
        )
    ...
    if not success:
        logger.error(
            f"Failed to delete chat room: admin_id={user_id}, room_id={room_id}"
        )
...
async def list_personas(request: Request, tab: str = "my"):
    ...
    if is_admin:
        ...
        personas = await get_user_personas(
            db_user.id, include_public=False
        )  # My personas only
    ...
    return templates.TemplateResponse(
        ...
        "personas.html",
        get_template_context(
            request, user_data, {"personas": personas, "active_tab": tab}
        ),
    )
...
async def new_persona(request: Request):
    ...
    return templates.TemplateResponse(
        request,
        "persona_edit.html",
        get_template_context(request, user_data, {"persona": None}),
    )
...
async def edit_persona(request: Request, persona_id: str):
    ...
    return templates.TemplateResponse(
        request,
        "persona_edit.html",
        get_template_context(request, user_data, {"persona": persona}),
    )
...
async def admin_dashboard(request: Request):
    ...
    return templates.TemplateResponse(
        ...
        get_template_context(request, user_data, {"stats": stats}),
    )
...
async def view_persona(request: Request, persona_id: str):
    ...
    if db_user:
        persona = await get_persona_by_id(
            persona_id, user_id=db_user.id
        )  # user_id is checked inside if owner, but if public it should return too.
    ...
    return templates.TemplateResponse(
        ...
        get_template_context(
            ...
            user_data,
            {
                ...
                "is_owner": str(persona.user_id) == str(db_user.id)
                if db_user
                else False,
            },
        ),
...
async def evaluate_persona_web(
    request: Request, persona_id: str, score: int = Form(...), comment: str = Form(None)
):
    user_data = get_current_user(request)
    if not user_data:
        return RedirectResponse(url="/login", status_code=302)
...
@router.post("/personas/{persona_id}/duplicate", response_class=HTMLResponse)
async def duplicate_persona_web(request: Request, persona_id: str):
    """Duplicate a persona (owner only)."""
    user_data = get_current_user(request)
    if not user_data:
        return RedirectResponse(url="/login", status_code=302)
    ...
    duplicated = await duplicate_persona(persona_id=persona_id, user_id=db_user.id)
    if not duplicated:
        raise HTTPException(
...
@router.post("/personas/bulk", response_class=HTMLResponse)
async def bulk_operation_web(
    request: Request,
    persona_ids: str = Form(...),  # Comma-separated IDs
    action: str = Form(...),  # 'delete', 'make_public', 'make_private'
):
    """Handle bulk operations on personas."""
    user_data = get_current_user(request)
    if not user_data:
        return RedirectResponse(url="/login", status_code=302)
    ...
    # Parse persona_ids from comma-separated string
...
@router.get("/personas/export", response_class=HTMLResponse)
async def export_page(request: Request):
    """Render the persona export page."""
    user_data = get_current_user(request)
    if not user_data:
        return RedirectResponse(url="/login")
    ...
    personas = []
    if db_user:
        personas = await get_user_personas(db_user.id, include_public=False)

    return templates.TemplateResponse(
        request,
        "persona_export.html",
        get_template_context(request, user_data, {"personas": personas}),
    )


@router.post("/personas/import", response_class=HTMLResponse)
async def import_persona_web(
    request: Request,
    personas_json: str = Form(...),  # JSON string
    overwrite_names: bool = Form(False),
):
